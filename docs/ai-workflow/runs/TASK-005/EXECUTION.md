...
| FINDING-005 (Round 2): Race condition ghi `.tsbuildinfo` giữa `typecheck` và `build` khi bật composite | Cấu hình `typecheck` trong `turbo.json` phụ thuộc tuần tự vào `build` (`"dependsOn": ["^build", "build"]`) để triệt tiêu hoàn toàn race condition ghi `.tsbuildinfo`.                               | `turbo.json`, `pnpm check`                        |
| FINDING-006 (Round 2): Tuyên bố phạm vi quality gate DoD trong AGENTS.md                               | Làm rõ trong `AGENTS.md` rằng M0 baseline bao gồm format, lint, typecheck, test, build; các gate mở rộng (OpenAPI drift, bundle budget, secret scan) nằm trong TASK-006 & TASK-007.                 | `AGENTS.md`                                       |
