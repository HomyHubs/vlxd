...
| FINDING-005 (Round 2 & 3): Publish GHCR, pull-based staging deploy, an toàn shell injection, và Promise.allSettled                       | Cấu hình publish GHCR với `${{ github.sha }}`, job deploy pull SHA image chính xác, an toàn injection bằng regex validation, và dùng `Promise.allSettled` trong smoke test. | `.github/workflows/deploy-staging.yml`, `scripts/smoke-test.mjs`           |
