...
| FINDING-005 (Round 2): Cấu hình deployment artifact SHA, rollback guard và kiểm thử smoke đồng thời                                      | Nâng cấp `deploy-staging.yml` xây dựng image theo commit SHA, triển khai kiểm thử smoke test đồng thời cả 2 services trong `smoke-test.mjs`, và cơ chế rollback tự động khi deploy fail. | `.github/workflows/deploy-staging.yml`, `scripts/smoke-test.mjs`, `compose.staging.yml` |
