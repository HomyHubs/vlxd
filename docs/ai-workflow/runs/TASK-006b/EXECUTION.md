...
| FINDING-005 (Round 2-5): Chu kỳ phát hành GHCR, Compose `image:` directives, bảo toàn release trước, và promotion sau kiểm thử           | Bổ sung `image: ${API_IMAGE}` / `image: ${WEB_IMAGE}` vào `compose.staging.yml`, gắn tag candidate trước khi deploy, bảo toàn `:staging-previous` trước khi promote `:staging`, rollback xác thực sức khỏe không dùng error masking, và assert Compose image resolution. | `compose.staging.yml`, `.github/workflows/deploy-staging.yml`, `scripts/smoke-test.mjs` |
