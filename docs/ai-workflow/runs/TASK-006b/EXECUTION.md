...
| FINDING-005 (Round 2-13): Chu kỳ phát hành GHCR, Compose `image:` directives, OCI imagetools manifest promotion, digest integrity check, và initial bootstrap guard | Bổ sung `image: ${API_IMAGE}` / `image: ${WEB_IMAGE}` vào `compose.staging.yml`, gắn tag candidate trước khi deploy, dùng `docker buildx imagetools create` sao chép OCI manifest trực tiếp có compensation recovery, kiểm tra tính hợp lệ của manifest digests sau promotion, xử lý an toàn kịch bản bootstrap ban đầu khi rollback chưa có tag cũ, xuất và lưu `manifest.json` bằng `actions/upload-artifact@v4`, và thu gọn permissions về `contents: read`, `packages: write`. | `compose.staging.yml`, `.github/workflows/deploy-staging.yml`, `scripts/smoke-test.mjs` |
