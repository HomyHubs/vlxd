...
| FINDING-005 (Round 2-16): Chu kỳ phát hành GHCR, Compose `image:` directives, OCI imagetools manifest promotion, digest equality check & full compensation rollback, và initial bootstrap guard | Bổ sung `image: ${API_IMAGE}` / `image: ${WEB_IMAGE}` vào `compose.staging.yml`, gắn tag candidate trước khi deploy, dùng `docker buildx imagetools create` sao chép OCI manifest trực tiếp có compensation recovery đa tầng, so khớp chính xác SHA256 digest giữa candidate và tag `:staging` kèm compensation rollback nếu mismatch, xử lý an toàn kịch bản bootstrap ban đầu khi rollback chưa có tag cũ, xuất và lưu `manifest.json` bằng `actions/upload-artifact@v4`, và thu gọn permissions về `contents: read`, `packages: write`. | `compose.staging.yml`, `.github/workflows/deploy-staging.yml`, `scripts/smoke-test.mjs` |
