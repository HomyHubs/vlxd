...
| FINDING-005 (Round 2-10): Chu kỳ phát hành GHCR, Compose `image:` directives, OCI imagetools atomic manifest promotion, và release manifest | Bổ sung `image: ${API_IMAGE}` / `image: ${WEB_IMAGE}` vào `compose.staging.yml`, gắn tag candidate trước khi deploy, dùng `docker buildx imagetools create` để copy OCI manifest nguyên tử không re-push layer, tự động backup `:staging-previous` khi có cả 2 dịch vụ, xuất `release-manifest.json`, đối soát so khớp toàn bộ tập images bằng sorted comparison, và hỗ trợ hook remote persistent staging khi config secrets. | `compose.staging.yml`, `.github/workflows/deploy-staging.yml`, `scripts/smoke-test.mjs` |
