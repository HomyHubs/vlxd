...
| FINDING-005 (Round 2-6): Chu kỳ phát hành GHCR, Compose `image:` directives, compensation rollback khi promotion lỗi, và assertion đối soát chính xác | Bổ sung `image: ${API_IMAGE}` / `image: ${WEB_IMAGE}` vào `compose.staging.yml`, gắn tag candidate trước khi deploy, bảo toàn `:staging-previous` trước khi promote `:staging`, compensation rollback tự động khôi phục nếu push Web lỗi, và đối soát so khớp toàn bộ tập images bằng sorted comparison. | `compose.staging.yml`, `.github/workflows/deploy-staging.yml`, `scripts/smoke-test.mjs` |
