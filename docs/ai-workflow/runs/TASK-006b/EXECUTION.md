...
| FINDING-005 (Round 2-15): Chu kỳ phát hành GHCR, Compose `image:` directives, OCI imagetools manifest promotion, digest equality check & rollback, initial bootstrap guard, và tracking task TASK-035 | Bổ sung `image: ${API_IMAGE}` / `image: ${WEB_IMAGE}` vào `compose.staging.yml`, gắn tag candidate trước khi deploy, dùng `docker buildx imagetools create` sao chép OCI manifest trực tiếp có compensation recovery, so khớp chính xác SHA256 digest giữa candidate và tag `:staging` kèm compensation rollback nếu mismatch, xử lý an toàn kịch bản bootstrap ban đầu khi rollback chưa có tag cũ, xuất và lưu `manifest.json` bằng `actions/upload-artifact@v4`, thu gọn permissions về `contents: read`, `packages: write`, và bổ sung task `TASK-035` persistent staging host follow-up trong `MVP-BACKLOG.md`. | `compose.staging.yml`, `.github/workflows/deploy-staging.yml`, `scripts/smoke-test.mjs`, `docs/tasks/MVP-BACKLOG.md` |
