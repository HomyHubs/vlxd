...
| FINDING-005 (Round 2-11): Chu kỳ phát hành GHCR, Compose `image:` directives, OCI imagetools manifest promotion, và release manifest artifact | Bổ sung `image: ${API_IMAGE}` / `image: ${WEB_IMAGE}` vào `compose.staging.yml`, gắn tag candidate trước khi deploy, dùng `docker buildx imagetools create` sao chép OCI manifest trực tiếp, xuất và lưu `manifest.json` bằng `actions/upload-artifact@v4`, và thu gọn permissions về `contents: read`, `packages: write`. | `compose.staging.yml`, `.github/workflows/deploy-staging.yml`, `scripts/smoke-test.mjs` |
