...
| FINDING-005 (Round 2, 3, 4): Chu kỳ phát hành GHCR, Compose `image:` directives, kiểm thử rollback không nuốt lỗi, và promotion sau kiểm thử | Bổ sung `image: ${API_IMAGE}` / `image: ${WEB_IMAGE}` vào `compose.staging.yml`, gắn tag candidate trước khi deploy, rollback xác thực sức khỏe không dùng ` |                                                                            | true`, và promote `:staging`/`:staging-previous` sau khi smoke test đạt. | `compose.staging.yml`, `.github/workflows/deploy-staging.yml`, `scripts/smoke-test.mjs` |
