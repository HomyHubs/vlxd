...
- [ ] Migration SQL thuần túy tại `db/migrations/` cho `tenants`, `users`, `tenant_users`, `sessions`
...
- [ ] Foreign keys, indexes, unique constraints (`uq_tenant_users_tenant_user`, token_hash, code, email)
...
- [ ] Execution log (`docs/ai-workflow/runs/TASK-008a/EXECUTION.md`)
...
| -------------------------------------------------------------------------------------- | ---------------------- | -------- |
| Migration SQL cho `tenants`, `users`, `tenant_users`, `sessions` tuân thủ chuẩn dbmate | Not verified           |          |
| Có cả `-- migrate:up` và `-- migrate:down` reversible sạch sẽ                          | Not verified           |          |
| Toàn bộ cột thời gian dùng UTC `timestamptz`                                           | Not verified           |          |
| Có trường `archived_at` phục vụ soft delete / archive                                  | Not verified           |          |
